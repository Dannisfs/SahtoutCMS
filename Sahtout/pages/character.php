            ...
            ");
                if (!$stmt) {
                    ...
                    if (!$stmt->execute()) {
                        ...
                        if (!$stats) {
                            // Make stats optional: Log warning but don't block page rendering
                            error_log("character.php: No stats found for guid=$guid (Table character_stats is empty or missing row)");
                            $stats = null;
                        }
                        $stmt->close();
                    }
                    ...
                    <div class="slot<?= isset($items[$slot]) ? ' has-item' : '' ?>" <?= isset($items[$slot]) ? 'data-tooltip="' . htmlspecialchars(generateTooltip($items[$slot])) . '"' : '' ?>>
                        ...
                        <div class="slot-info">
                            ...
                            <?php if (isset($items[$slot])): ?>
                                <div class="slot-item"
                                    style="color:<?= $qualityColors[$items[$slot]['Quality']] ?? '#ffffff' ?>">
            ...
            <div class="character-center">
                ...
                <?php else: ?>
                    ...
                    <div class="character-details">
                        <span class="character-level"><?php echo translate('level_label', 'Level'); ?>
                            <?= $character['level'] ?? '??' ?>
                            <?= isset($classes[$character['class']]) ? htmlspecialchars($classes[$character['class']]['name']) : translate('class_unknown', 'Unknown') ?></span>
                        <span
                            class="character-race"><?= isset($races[$character['race']]) ? htmlspecialchars($races[$character['race']]['name']) : translate('race_unknown', 'Unknown') ?></span>
                    </div>
                    <div class="character-image">
                        <img src="<?php echo $base_path; ?>3dmodels/3d_default.gif"
                    ...
                    <div class="stats-container">
                        <div class="stats-category">
                            ...
                            <div class="stats-item">
                                <span><?php echo translate('stat_health', 'Health'); ?></span><span><?= number_format($stats['maxhealth']) ?></span>
                            </div>
                            <?php
                            ...
                            if ($stats['maxpower1'] > 0):
                                ?>
                                <div class="stats-item">
                                    <span><?php echo translate('stat_mana', 'Mana'); ?></span><span><?= number_format($stats['maxpower1']) ?></span>
                                </div>
                            <?php else: ?>
                                <div class="stats-item">
                                    <span><?php echo translate('stat_mana', 'Mana'); ?></span><span><?php echo translate('stat_not_available', 'Not Available'); ?></span>
                                </div>
                            <?php endif; ?>
                            <?php
                            ...
                            if ($powerIndex > 0 && $stats["maxpower" . ($powerIndex + 1)] > 0):
                                ?>
                                <div class="stats-item">
                                    <span><?= htmlspecialchars($powerTypes[$powerIndex]) ?></span><span><?= number_format($displayPowerValue) ?></span>
                                </div>
                            <?php endif; ?>
                            <div class="stats-item">
                                <span><?php echo translate('stat_strength', 'Strength'); ?></span><span><?= number_format($stats['strength']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_agility', 'Agility'); ?></span><span><?= number_format($stats['agility']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_stamina', 'Stamina'); ?></span><span><?= number_format($stats['stamina']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_intellect', 'Intellect'); ?></span><span><?= number_format($stats['intellect']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_spirit', 'Spirit'); ?></span><span><?= number_format($stats['spirit']) ?></span>
                            </div>
                        </div>
                        <div class="stats-category">
                            <h3><?php echo translate('stats_defense', 'Defense'); ?></h3>
                            <div class="stats-item">
                                <span><?php echo translate('stat_armor', 'Armor'); ?></span><span><?= number_format($stats['armor']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_block', 'Block'); ?></span><span><?= number_format($stats['blockPct'], 2) ?>%</span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_dodge', 'Dodge'); ?></span><span><?= number_format($stats['dodgePct'], 2) ?>%</span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_parry', 'Parry'); ?></span><span><?= number_format($stats['parryPct'], 2) ?>%</span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_resilience', 'Resilience'); ?></span><span><?= number_format($stats['resilience']) ?></span>
                            </div>
                        </div>
                        <div class="stats-category">
                            <h3><?php echo translate('stats_melee', 'Melee'); ?></h3>
                            <div class="stats-item">
                                <span><?php echo translate('stat_attack_power', 'Attack Power'); ?></span><span><?= number_format($stats['attackPower']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_crit_chance', 'Crit Chance'); ?></span><span><?= number_format($stats['critPct'], 2) ?>%</span>
                            </div>
                        </div>
                        <div class="stats-category">
                            <h3><?php echo translate('stats_ranged', 'Ranged'); ?></h3>
                            <div class="stats-item">
                                <span><?php echo translate('stat_ranged_attack_power', 'Attack Power'); ?></span><span><?= number_format($stats['rangedAttackPower']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_ranged_crit_chance', 'Crit Chance'); ?></span><span><?= number_format($stats['rangedCritPct'], 2) ?>%</span>
                            </div>
                        </div>
                        <div class="stats-category">
                            <h3><?php echo translate('stats_resistances', 'Resistances'); ?></h3>
                            <div class="stats-item">
                                <span><?php echo translate('stat_holy_resistance', 'Holy Resistance'); ?></span><span><?= number_format($stats['resHoly']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_fire_resistance', 'Fire Resistance'); ?></span><span><?= number_format($stats['resFire']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_nature_resistance', 'Nature Resistance'); ?></span><span><?= number_format($stats['resNature']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_frost_resistance', 'Frost Resistance'); ?></span><span><?= number_format($stats['resFrost']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_shadow_resistance', 'Shadow Resistance'); ?></span><span><?= number_format($stats['resShadow']) ?></span>
                            </div>
                            <div class="stats-item">
                                <span><?php echo translate('stat_arcane_resistance', 'Arcane Resistance'); ?></span><span><?= number_format($stats['resArcane']) ?></span>
                            </div>
                        </div>
                    </div>
                <?php else: ?>
                    ...
                    <?php foreach ($pvp_teams as $team): ?>
                        <div class="pvp-team-item">
                            <div class="pvp-team">
                                ...
                                (<?= $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : '5v5') ?>,
                                <?php echo translate('pvp_rating', 'Rating'); ?>: <?= $team['rating'] ?>)
                            </div>
