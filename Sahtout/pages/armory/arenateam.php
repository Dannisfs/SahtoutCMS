...
<div class="arena-content tw-bg-gray-900 tw-text-white">
    <div
        class="tw-container tw-max-w-6xl tw-mx-auto tw-ml-0 tw-mr-auto sm:tw-mx-auto tw-px-2 tw-py-4 sm:tw-px-4 sm:tw-py-8">
        <?php if (!$team): ?>
            <div
                class="tw-text-center tw-text-base sm:tw-text-lg tw-text-amber-400 tw-bg-gray-800 tw-p-4 sm:tw-p-6 tw-rounded-lg tw-shadow-lg tw-max-w-6xl tw-mx-auto">
                <?php echo translate('arenateam_no_team', 'No arena team found.'); ?>
            </div>
        <?php else: ?>
            <h1
                class="team-header tw-text-[2rem] sm:tw-text-[2.5rem] tw-font-bold tw-text-center tw-text-gold-300 tw-mb-6 tw-p-2 sm:tw-p-4 tw-rounded-xl tw-max-w-6xl tw-mx-auto">
                <img src="<?php echo $base_path; ?>img/armory/arena.webp" alt="Arena Team" title="Arena Team"
                    class="arena-icon inline-block">
                <?php echo htmlspecialchars($team['team_name']); ?> - <?php echo getTeamTypeName($team['type']); ?>
            ...
            <div class="summary-container tw-p-4 sm:tw-p-6 tw-rounded-lg tw-shadow-lg tw-mb-8 tw-max-w-6xl tw-mx-auto">
                <h2 class="tw-text-xl sm:tw-text-2xl tw-font-bold tw-text-amber-400 tw-mb-4">
                ...
                <div class="tw-grid tw-grid-cols-2 sm:tw-grid-cols-4 tw-gap-4 tw-text-center">
                    <div
                        class="summary-item summary-item-<?php echo $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : ($team['type'] == 5 ? '5v5' : 'default')); ?> tw-p-2 sm:tw-p-3 tw-rounded-lg">
                        <p class="tw-text-base sm:tw-text-lg tw-text-gray-300">
                            <?php echo translate('arenateam_rating', 'Rating'); ?>
                        </p>
                        <p class="tw-text-lg sm:tw-text-xl tw-font-semibold tw-text-gold-300 summary-value">
                            <?php echo $team['rating']; ?>
                        </p>
                    </div>
                    <div
                        class="summary-item summary-item-<?php echo $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : ($team['type'] == 5 ? '5v5' : 'default')); ?> tw-p-2 sm:tw-p-3 tw-rounded-lg">
                        <p class="tw-text-base sm:tw-text-lg tw-text-gray-300">
                            <?php echo translate('arenateam_winrate', 'Winrate'); ?>
                        </p>
                        <p class="tw-text-lg sm:tw-text-xl tw-font-semibold tw-text-gold-300 summary-value">
                            <?php echo $team['winrate']; ?>%
                        </p>
                    </div>
                    <div
                        class="summary-item summary-item-<?php echo $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : ($team['type'] == 5 ? '5v5' : 'default')); ?> tw-p-2 sm:tw-p-3 tw-rounded-lg">
                        <p class="tw-text-base sm:tw-text-lg tw-text-gray-300">
                            <?php echo translate('arenateam_season_games', 'Season Games'); ?>
                        </p>
                        <p class="tw-text-lg sm:tw-text-xl tw-font-semibold tw-text-gold-300 summary-value">
                            <?php echo $team['seasonGames']; ?>
                        </p>
                    </div>
                    <div
                        class="summary-item summary-item-<?php echo $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : ($team['type'] == 5 ? '5v5' : 'default')); ?> tw-p-2 sm:tw-p-3 tw-rounded-lg">
                        <p class="tw-text-base sm:tw-text-lg tw-text-gray-300">
                            <?php echo translate('arenateam_season_wins', 'Season Wins'); ?>
                        </p>
                        <p class="tw-text-lg sm:tw-text-xl tw-font-semibold tw-text-gold-300 summary-value">
                            <?php echo $team['seasonWins']; ?>
                        </p>
                    </div>
                    <div
                        class="summary-item summary-item-<?php echo $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : ($team['type'] == 5 ? '5v5' : 'default')); ?> tw-p-2 sm:tw-p-3 tw-rounded-lg">
                        <p class="tw-text-base sm:tw-text-lg tw-text-gray-300">
                            <?php echo translate('arenateam_season_losses', 'Season Losses'); ?>
                        </p>
                        <p class="tw-text-lg sm:tw-text-xl tw-font-semibold tw-text-gold-300 summary-value">
                            <?php echo $team['seasonLosses']; ?>
                        </p>
                    </div>
                    <div
                        class="summary-item summary-item-<?php echo $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : ($team['type'] == 5 ? '5v5' : 'default')); ?> tw-p-2 sm:tw-p-3 tw-rounded-lg">
                        <p class="tw-text-base sm:tw-text-lg tw-text-gray-300">
                            <?php echo translate('arenateam_week_games', 'Week Games'); ?>
                        </p>
                        <p class="tw-text-lg sm:tw-text-xl tw-font-semibold tw-text-gold-300 summary-value">
                            <?php echo $team['weekGames'] ?? 'N/A'; ?>
                        </p>
                    </div>
                    <div
                        class="summary-item summary-item-<?php echo $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : ($team['type'] == 5 ? '5v5' : 'default')); ?> tw-p-2 sm:tw-p-3 tw-rounded-lg">
                        <p class="tw-text-base sm:tw-text-lg tw-text-gray-300">
                            <?php echo translate('arenateam_week_wins', 'Week Wins'); ?>
                        </p>
                        <p class="tw-text-lg sm:tw-text-xl tw-font-semibold tw-text-gold-300 summary-value">
                            <?php echo $team['weekWins'] ?? 'N/A'; ?>
                        </p>
                    </div>
                    <div
                        class="summary-item summary-item-<?php echo $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : ($team['type'] == 5 ? '5v5' : 'default')); ?> tw-p-2 sm:tw-p-3 tw-rounded-lg">
                        <p class="tw-text-base sm:tw-text-lg tw-text-gray-300">
                            <?php echo translate('arenateam_week_losses', 'Week Losses'); ?>
                        </p>
                        <p class="tw-text-lg sm:tw-text-xl tw-font-semibold tw-text-gold-300 summary-value">
                            <?php echo $team['weekLosses'] ?? 'N/A'; ?>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- Team Members -->
            <h2 class="tw-text-xl sm:tw-text-2xl tw-font-bold tw-text-amber-400 tw-mb-4 tw-max-w-6xl tw-mx-auto">
            ...
            <div class="table-container tw-overflow-x-auto tw-rounded-lg tw-shadow-lg tw-max-w-6xl tw-mx-auto">
                <table class="tw-w-full tw-text-xs sm:tw-text-sm tw-text-center tw-bg-gray-900/90">
                    <thead class="tw-text-gold-300 tw-uppercase"
                        style="background: linear-gradient(to right, #4338ca, #1e1b4b);">
                        <tr>
                            <th class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                <?php echo translate('arenateam_name', 'Name'); ?>
                            </th>
                            <th class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                <?php echo translate('arenateam_faction', 'Faction'); ?>
                            </th>
                            <th class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                <?php echo translate('arenateam_race', 'Race'); ?>
                            </th>
                            <th class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                <?php echo translate('arenateam_class', 'Class'); ?>
                            </th>
                            <th class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                <?php echo translate('arenateam_personal_rating', 'Personal Rating'); ?>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (count($orderedMembers) == 0): ?>
                            <tr>
                                <td colspan="5"
                                    class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6 tw-text-base sm:tw-text-lg tw-text-amber-400">
                        ...
                        <?php else: ?>
                            <?php foreach ($orderedMembers as $member): ?>
                                ...
                                <tr class="<?php echo $member['guid'] == $team['captainGuid'] ? 'captain-row' : ''; ?> tw-transition tw-duration-200"
                                    onclick="window.location='<?php echo $base_path; ?>pages/character.php?guid=<?php echo $member['guid']; ?>';">
                                    <td class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                        <?php if ($member['guid'] == $team['captainGuid']): ?>
                                            <img src="<?php echo $base_path; ?>img/armory/leader.png" alt="Team Captain"
                                                title="Team Captain" class="leader-icon inline-block">
                                        <?php endif; ?>
                                        <?php echo htmlspecialchars($member['name']); ?>
                                    </td>
                                    <td class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                        <img src="<?php echo factionIconByName($faction); ?>" alt="<?php echo $faction; ?>"
                                            title="<?php echo $faction; ?>"
                                            class="tw-inline-block tw-w-5 tw-h-5 sm:tw-w-6 sm:tw-h-6 tw-rounded">
                                    </td>
                                    <td class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                        <img src="<?php echo raceIcon($member['race'], $member['gender']); ?>" alt="Race"
                                            class="tw-inline-block tw-w-5 tw-h-5 sm:tw-w-6 sm:tw-h-6 tw-rounded">
                                    </td>
                                    <td class="tw-py-2 tw-px-4 sm:tw-py-3 sm:tw-px-6">
                                        <img src="<?php echo classIcon($member['class']); ?>" alt="Class"
                                            class="tw-inline-block tw-w-5 tw-h-5 sm:tw-w-6 sm:tw-h-6 tw-rounded">
                                    </td>
