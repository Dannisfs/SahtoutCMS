...
require_once $project_root . 'includes/config.settings.php'; // Include settings for title
...
if ($is_single) {
    ...
    if (!$news) {
        ...
        echo '<div class="main-content"><div class="wow-news-container">';
            ...
            <article class="news-single <?php echo $news['is_important'] ? 'important' : ''; ?>">
                <?php if (!empty($news['image_url'])): ?>
                    <img src="<?php echo $base_path . htmlspecialchars($news['image_url']); ?>"
                        alt="<?php echo htmlspecialchars($news['title']); ?>" class="news-single-image"
                ...
                <div class="news-single-meta">
                    ...
                    <span
                        class="author"><?php echo sprintf(translate('posted_by', 'Posted by %s'), htmlspecialchars($news['posted_by'])); ?></span>
                </div>
                <div class="news-single-content">
                    <?php echo nl2br(htmlspecialchars($news['content'])); ?>
                </div>
                <a href="<?php echo $base_path; ?>news"
                    class="news-single-back"><?php echo translate('back_to_news', '← Back to News'); ?></a>
            </article>
        <?php else: ?>
            <!-- News List -->
                ...
                <?php if ($total_pages > 1): ?>
                    <div class="news-pagination">
                        <?php if ($current_page > 1): ?>
                            <a href="<?php echo $base_path; ?>news?page=<?php echo $current_page - 1; ?>" class="pagination-link"
                                aria-label="<?php echo translate('pagination_previous', 'Previous page'); ?>">«
                                <?php echo translate('pagination_prev', 'Prev'); ?></a>
                        <?php endif; ?>
                        ...
                        <?php if ($current_page < $total_pages): ?>
                            <a href="<?php echo $base_path; ?>news?page=<?php echo $current_page + 1; ?>" class="pagination-link"
                                aria-label="<?php echo translate('pagination_next_label', 'Next page'); ?>"><?php echo translate('pagination_next', 'Next'); ?>
                                »</a>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
