...
<body>
                ...
                <div class="content">
                    ...
                    <?php if ($success): ?>
                        <div class="success-box mb-3 col-md-6 mx-auto">
                            ...
                            <span
                                class="success"><?php echo translate('msg_realm_saved', 'Realm configuration saved successfully!'); ?></span>
                        </div>
                    <?php endif; ?>
                    ...
                    <form method="post" enctype="multipart/form-data">
                        <div class="mb-3 col-md-6 mx-auto">
                            <label for="realm_name"
                                class="form-label"><?php echo translate('label_realm_name', 'Realm Name'); ?></label>
                            <input type="text" class="form-control" id="realm_name" name="realm_name"
                        ...
                        <div class="mb-3 col-md-6 mx-auto">
                            <label for="realm_ip"
                                class="form-label"><?php echo translate('label_realm_ip', 'Realm IP / Host'); ?></label>
                            <input type="text" class="form-control" id="realm_ip" name="realm_ip"
                                placeholder="127.0.0.1"
                                value="<?php echo htmlspecialchars($_POST['realm_ip'] ?? $currentRealm['address']); ?>"
                                required>
                        </div>
                        <div class="mb-3 col-md-6 mx-auto">
                            <label for="realm_port"
                                class="form-label"><?php echo translate('label_realm_port', 'Realm Port'); ?></label>
                            <input type="number" class="form-control" id="realm_port" name="realm_port"
                                placeholder="8085"
                                value="<?php echo htmlspecialchars($_POST['realm_port'] ?? $currentRealm['port']); ?>"
                                required>
                        </div>
                        <div class="mb-3 col-md-6 mx-auto">
                            <label for="realm_logo"
                                class="form-label"><?php echo translate('label_realm_logo', 'Realm Logo'); ?></label>
                            <div class="custom-file-upload">
                                <input type="file" id="realm_logo" name="realm_logo"
                                    accept="image/png,image/svg+xml,image/jpeg,image/webp">
                                <button type="button" class="btn btn-outline-secondary"
                                    onclick="document.getElementById('realm_logo').click();"><?php echo translate('btn_choose_file', 'Choose File'); ?></button>
                                <div class="file-name" id="file-name-realm">
                                    <?php echo translate('placeholder_realm_logo', 'Upload a PNG, SVG, JPG, or WebP image (max 1GB) for the realm logo.'); ?>
                                </div>
                            </div>
                        </div>
                        <div class="form-text mb-3">
                            <?php echo translate('note_realm_config', 'Note: This configures the settings for a single realm.'); ?>
                        </div>
                        <button type="submit"
                            class="btn btn-primary"><?php echo translate('btn_save_realm', 'Save Realm Configuration'); ?></button>
                    </form>
                </div>
            </main>
        </div>
    ...
    <script>
        ...
        document.getElementById('realm_logo').addEventListener('change', function () {
            const fileName = this.files.length > 0 ? this.files[0].name : '<?php echo translate('placeholder_realm_logo', 'Upload a PNG, SVG, JPG, or WebP image (max 1GB) for the realm logo.'); ?>';
